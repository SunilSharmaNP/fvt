...
async def start_merge_task(client: Client, trigger_message: Message,
    ...
    try:
        ...
        await log_manager.send_stage_notification(client, task_id, "Download Started", f"{len(messages_to_merge)} files")
        ...
        await log_manager.send_stage_notification(client, task_id, "Processing Started")
        ...
        await log_manager.send_stage_notification(client, task_id, "Upload Started", f"to {upload_choice}")
        ...
        if upload_choice == "gofile":
            ...
            await log_manager.finish_task_log(client, log_message_id, "Complete", file_size, gofile_link, task_id)
        else:
            ...
            await log_manager.finish_task_log(client, log_message_id, "Complete", final_size, None, task_id)
        ...
        if status_message and "Processing failed" not in str(e):
            try:
                ...
                pass
        if log_message_id:
            await log_manager.finish_task_log(client, log_message_id, f"Failed: {str(e)}", 0, None, task_id)
...
async def start_processing_task(client: Client, message: Message, user_id: int,
    ...
    try:
        ...
        log_message_id = await log_manager.create_task_log(client, user, settings, task_id)
        await db.update_task(task_id, {"status": "downloading"})
        await log_manager.send_stage_notification(client, task_id, "Download Started")
        ...
        if not output_file_path:
            if settings['active_tool'] == 'mediainfo':
                await log_manager.finish_task_log(client, log_message_id, "Complete (MediaInfo)", 0, None, task_id)
                await db.update_task(task_id, {"status": "completed"})
                return
            else:
                raise Exception("Processing failed. Check status message for error.")
