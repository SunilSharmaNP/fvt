...
# ✅ Fixed: User Settings Downloading/Uploading buttons logic corrected.
# ✅ Updated: Added handlers for Screenshot, HD Cover, and Audio Remover menus.
...
from modules.ui_menus import (get_start_menu, get_user_settings_menu,
                              get_video_tools_menu, get_admin_menu)
    ...
    try:
        ...
        if panel_type == "start":
            ...
            if panel_type == "us:metadata":
                ...
                image, caption, keyboard = await get_download_mode_submenu(
                    user_id)
            elif panel_type == "us:mode:upload":
                image, caption, keyboard = await get_upload_mode_submenu(
                    user_id)
            ...
            from modules.ui_menus import (
                ...
                get_vt_screenshot_menu, get_vt_audioremover_menu,
                get_vt_hdcover_menu, get_vt_speed_menu, get_vt_volume_menu,
            ...
            menu_map = {
                ...
                "screenshot": get_vt_screenshot_menu,
                "audioremover": get_vt_audioremover_menu,
                "hdcover": get_vt_hdcover_menu
            }

            if tool in menu_map:
                image, caption, keyboard = await menu_map[tool](user_id,
                                                                submenu)
...
def register_callback_handlers(app: Client):
    ...
    async def callback_handler(client: Client, query: CallbackQuery):
        ...
        # debug: log raw callback data for troubleshooting if needed
        logger.debug(f"Callback from user {user_id} in chat {chat_id}: {data}")
        ...
        try:
            ...
            if data.startswith("queue:"):
                ...
                if action == "add_more":
                    ...
                    if not queue_manager.has_queue(
                            user_id) or queue_manager.get_queue_count(
                                user_id) < 2:
                        return await query.answer(
                            "❌ Need at least 2 videos in queue",
                            show_alert=True)
                    ...
                    messages_to_merge = [
                        item['message'] for item in queue_items
                    ]
            ...
            if data.startswith("open:"):
                ...
                if panel in ["start", "settings", "tools", "admin"]:
                    ...
                    await query.message.edit_media(
                        InputMediaPhoto(config.IMG_START,
                                        caption=config.MSG_HELP),
                    ...
                    caption = config.MSG_ABOUT.format(
            ...
            if prefix == "us":
                ...
                # ✅ FIXED: Check if the action is "mode" and payload indicates "open"
                if action == "mode":
                    # Handle mode submenus (download/upload)
                    # Example data: us:mode:download:open
                    # Payload will be: download:open
                    if payload.endswith(":open"):
                        # Extracts 'download' or 'upload'
                        mode_type = payload.replace(":open", "")
                        return await refresh_panel(query,
                                                   f"us:mode:{mode_type}")
                    ...
                    if key in ["download_mode", "upload_mode"]:
                        ...
                        await query.answer(
                            f"{key.replace('_',' ').title()} set to {value.capitalize()}"
                        )
                    ...
                    await db.update_user_setting(user_id, "custom_thumbnail",
                                                 None)
                    await query.answer("Thumbnail cleared.")
                    ...
                    try:
                        if key == "custom_filename":
                            ...
                            await db.update_user_setting(
                                user_id, "custom_filename", r.text)
                            await r.reply_text(config.MSG_SET_SUCCESS)
                            ...
                            r = await client.ask(chat_id,
                                                 ...
                                                 timeout=300)
                            await db.update_user_setting(
                                user_id, "custom_thumbnail", r.photo.file_id)
                            await r.reply_text(config.MSG_SET_SUCCESS)
                    except asyncio.TimeoutError:
                        return await client.send_message(
                            chat_id, config.MSG_SET_TIMEOUT)
            ...
            if prefix == "vt":
                ...
                if action == "open":
                    ...
                    if active == tool:
                        ...
                        # Clear HD Cover cache when tool is disabled
                        if tool == "hdcover":
                            # Placeholder - you may remove if not using a module
                            pass

                        await query.answer(
                            f"{tool.capitalize()} tool disabled.")
                    else:
                        await db.update_user_setting(user_id, "active_tool",
                                                     tool)
                        await query.answer(
                            f"{tool.capitalize()} tool enabled ✅")
                    return await refresh_panel(query, f"vt:{tool}:main")
                elif action == "set":
                    # payload like "key:value"
                    key, value = payload.split(":", 1)
                    # Merge mode special-case
                    if tool == "merge":
                        await db.update_user_setting(user_id, "merge_mode",
                                                     value)
                        return await refresh_panel(query, "vt:merge:main")

                    # Screenshot: count set via direct buttons (already handled here)
                    if tool == "screenshot" and key == "count":
                        count = int(value)
                        await db.update_user_nested_setting(
                            user_id, "screenshot_settings.count", count)
                        await query.answer(f"📸 Screenshot count set to {count}"
                                           )
                        return await refresh_panel(query, "vt:screenshot:main")

                    # Audio Remover: set mode (remove/keep)
                    if tool == "audioremover" and key == "mode":
                        await db.update_user_nested_setting(
                            user_id, "audioremover_settings.mode", value)
                        await query.answer(
                            f"Audio remover mode set to {value.capitalize()}")
                        return await refresh_panel(query,
                                                   "vt:audioremover:main")

                    # Extract thumbnail: set mode
                    if tool == "extract_thumb" and key == "mode":
                        await db.update_user_nested_setting(
                            user_id, "extract_thumb_settings.mode", value)
                        await query.answer(
                            f"Thumbnail extraction mode set to {value.capitalize()}"
                        )
                        return await refresh_panel(query,
                                                   "vt:extract_thumb:main")

                    # Generic nested key updates
                    ...
                    if key == "resolution":
                        if value.endswith("_hevc"):
                            ...
                            await query.answer(f"Set {value.upper()} (H.264)",
                                               show_alert=False)
                        return await refresh_panel(query,
                                                   f"vt:{tool}:resolution")
                    if key in [
                            "crf", "duration", "angle", "volume", "fps",
                            "scale", "count"
                    ]:
                        try:
                            value = int(value)
                    ...
                    if key in [
                            "vcodec", "crf", "preset", "resolution", "acodec"
                    ]:
                    ...
                    # payload is the key to ask for (e.g., "count", "timestamp", "upload")
                    key = payload
                    db_key = f"{tool}_settings.{key}"
                    ...
                    if tool == "encode":
                        if key == "crf":
                            ...
                            validation = lambda x: x.isdigit() and 0 <= int(
                                x) <= 51
                        elif key == "abitrate":
                    ...
                    # Screenshot custom count
                    elif tool == "screenshot" and key == "count":
                        ask_msg = "📸 Send desired screenshot count (number):"
                        error_msg = "❌ Invalid number."
                        validation = lambda x: x.isdigit() and int(x) > 0

                    # HD Cover upload (ask for photo)
                    elif tool == "hdcover" and key == "upload":
                        ask_msg = "📤 Please upload the cover image (photo):"
                        # We'll ask for a photo filter

                    # Audio Remover doesn't need free-text asks currently

                    # Extract thumbnail asks
                    elif tool == "extract_thumb":
                        if key == "timestamp":
                            ask_msg = "⏱ Send timestamp for thumbnail (format: HH:MM:SS or MM:SS or seconds):"
                            error_msg = "❌ Invalid timestamp."
                            validation = lambda x: parse_time_input(
                                x) is not None
                        elif key == "count":
                            ask_msg = "🔢 Send number of thumbnails to extract:"
                            error_msg = "❌ Invalid number."
                            validation = lambda x: x.isdigit() and int(x) > 0
                    ...
                    await query.answer()
                    try:
                        # HD Cover needs photo filter
                        if tool == "hdcover" and key == "upload":
                            r = await client.ask(chat_id,
                                                 ask_msg,
                                                 filters=filters.photo,
                                                 timeout=300)
                            # r.photo is a list; take highest quality
                            photo = r.photo
                            if not photo:
                                return await r.reply_text(
                                    "❌ No photo received.")
                            file_id = photo.file_id if hasattr(
                                photo, "file_id") else photo[-1].file_id
                            # Save temp cover (db API assumed)
                            try:
                                await db.save_temp_cover(user_id, file_id)
                                await r.reply_text("✅ Cover image saved.")
                            except AttributeError:
                                # fallback: save into a known key if save_temp_cover not implemented
                                await db.update_user_nested_setting(
                                    user_id, "hdcover_settings.file_id",
                                    file_id)
                                await r.reply_text(
                                    "✅ Cover image saved (using hdcover_settings.file_id)."
                                )
                            return await refresh_panel(query,
                                                       "vt:hdcover:main")
                        ...
                        if tool == "encode" and key == "resolution":
                            await db.update_user_nested_setting(
                                user_id, "encode_settings.resolution",
                                "custom")
                            await db.update_user_nested_setting(
                                user_id, db_key, val)
                        elif tool == "screenshot" and key == "count":
                            await db.update_user_nested_setting(
                                user_id, "screenshot_settings.count", int(val))
                        elif tool == "extract_thumb" and key == "count":
                            await db.update_user_nested_setting(
                                user_id, "extract_thumb_settings.count",
                                int(val))
                        elif tool == "extract_thumb" and key == "timestamp":
                            await db.update_user_nested_setting(
                                user_id, "extract_thumb_settings.timestamp",
                                val)
                        else:
                            await db.update_user_nested_setting(
                                user_id, db_key, val)
                    ...
                    if tool != "merge":
                        return await query.answer(
                            "Queue is only for merge tool!", show_alert=True)
                ...
                # --- HD COVER CLEAR LOGIC ---
                elif tool == "hdcover" and action == "clear":
                    # delete_temp_cover exists in ui_menus usage
                    try:
                        await db.delete_temp_cover(user_id)
                        await query.answer(
                            "🗑️ Cover image cleared from memory.")
                    except AttributeError:
                        # fallback: clear known nested key
                        await db.update_user_nested_setting(
                            user_id, "hdcover_settings.file_id", None)
                        await query.answer(
                            "🗑️ Cover image cleared from memory (fallback).")
                    return await refresh_panel(query, "vt:hdcover:main")
                ...
                if act == "toggle" and payload == "mode":
                    ...
                    if not process_manager.active_processes:
                        ...
                        for task_id, data in process_manager.active_processes.items(
                            ...
                            db_task = await db.get_task(task_id)
                            tool = db_task.get('tool',
                                               'N/A') if db_task else 'N/A'
                    ...
                    await query.answer(
                        "📣 Broadcast tooling under development.",
                        show_alert=True)
                    logger.info(
                    ...
                    if user_id not in config.SUDO_USERS:
                        return await query.answer(
                            "❌ Only Sudo Users can restart.", show_alert=True)
