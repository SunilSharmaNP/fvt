...
class Config:
    ...
    try:
        ...
        logger.warning(f"LOG_CHANNEL must be a valid integer, got: {_log_channel_raw}. Setting to None.")
        LOG_CHANNEL = None
    ...
    try:
        ...
        logger.warning(f"TASK_LOG_CHANNEL must be a valid integer, got: {_task_log_channel_raw}. Setting to None.")
...
def validate_config():
    ...
    try:
        ...
        if isinstance(Config.PROCESS_POLL_INTERVAL_S, str):
            ...
            Config.PROCESS_POLL_INTERVAL_S = int(Config.PROCESS_POLL_INTERVAL_S)
        ...
        if isinstance(Config.PROCESS_CANCEL_TIMEOUT_S, str):
            ...
            Config.PROCESS_CANCEL_TIMEOUT_S = int(Config.PROCESS_CANCEL_TIMEOUT_S)

        def to_int_list(var_str):
            if var_str:
                return [int(clean_value(x)) for x in var_str.split(",") if clean_value(x).lstrip('-').isdigit()]
            return []
