# bot.py (v6.0)
# MODIFIED for Granular 've' Repo UI (Step 4)
# 1. `callback_handler` aur `refresh_panel` ko naye granular menu (v6.0) ke liye poori tarah rewrite kiya gaya hai.
# 2. `client.ask` logic ko har granular setting (CRF, Resolution, Trim Start/End, etc.) ke liye implement kiya gaya hai.
# 3. Database calls ab `db.update_user_nested_setting("encode_settings.vcodec", ...)` ka istemaal karti hain.
# 4. `task_manager.py` (v5.3) se poori tarah hata diya gaya hai.
...
if not isinstance(config.API_ID, int):
    logger.critical("❌ Configuration not set! Please edit config.env.")
    sys.exit(1)
...
# Note: USER_FILE_QUEUE removed - now using queue_manager from modules/queue_manager.py
...
async def refresh_panel(query: CallbackQuery, panel_type: str):
    ...
    try:
        ...
        # MODIFIED: (v6.0) Naya routing logic
    ...
    try:
        ...
        if prefix == "vt":
            ...
            # 🔹 SET TOOL VALUES
                ...
                # ✅ Granular Tools (Encode, Trim, etc.)
                db_key = f"{tool}_settings.{key}"

                # 🆕 Special HEVC handling for resolution
                if key == "resolution":
