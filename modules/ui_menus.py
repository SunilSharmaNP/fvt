...
async def get_vt_screenshot_menu(user_id: int, menu_type: str = "main"):
    ...
    screenshot_settings = settings.get("screenshot_settings",
                                       default_settings['screenshot_settings'])

    # Safe defaults to avoid KeyError
    timestamp = screenshot_settings.get("timestamp", "00:00:00")
    count = screenshot_settings.get("count", 5)
    mode = screenshot_settings.get("mode", "single")

    active_tool = settings.get("active_tool")

    # ===================== MAIN MENU =====================
    if menu_type == "main":
        caption = config.MSG_VT_SCREENSHOT_MAIN.format(timestamp=timestamp,
                                                       count=count,
                                                       mode=mode)

        buttons = [
            InlineKeyboardButton(f"⏱ Timestamp: {timestamp}",
                                 callback_data="vt:screenshot:open:timestamp"),
            InlineKeyboardButton(f"🔢 Count: {count}",
                                 callback_data="vt:screenshot:open:count"),
            InlineKeyboardButton(f"🎯 Mode: {mode}",
                                 callback_data="vt:screenshot:open:mode"),
            InlineKeyboardButton(
    ...
    if menu_type == "count":
        current = count
        caption = "🔢 **Select number of screenshots:**"

        buttons = [
            InlineKeyboardButton(f"2 {tick(current == 2)}",
                                 callback_data="vt:screenshot:set:count:2"),
            InlineKeyboardButton(f"5 {tick(current == 5)}",
                                 callback_data="vt:screenshot:set:count:5"),
            InlineKeyboardButton(f"8 {tick(current == 8)}",
                                 callback_data="vt:screenshot:set:count:8"),
            InlineKeyboardButton(f"10 {tick(current == 10)}",
    ...
    # ===================== FALLBACK =====================
    return config.IMG_TOOLS, (
        "⚠️ **Invalid screenshot menu.**"), create_keyboard([
