...
async def get_video_tools_menu(user_id: int):
    ...
    buttons = [
        ...
        InlineKeyboardButton(
            f"🖼️ HD Cover {tick(active_tool == 'hdcover')}",
...
def _get_vt_watermark_main(settings: dict, active_tool: str):
    ...
    buttons = [
        ...
        # (Opacity can be added as another ask button)
        InlineKeyboardButton(
...
# =========================================================
# NEW VIDEO TOOLS SUBMENUS
# =========================================================

async def get_vt_screenshot_menu(user_id: int, menu_type: str = "main"):
    """Screenshot Generator Tool Menu"""
    settings = await db.get_user_settings(user_id)
    active_tool = settings.get("active_tool")
    
    caption = (
        "📸 **Screenshot Generator**\n\n"
        "Extract random screenshots from your videos!\n\n"
        "**How to use:**\n"
        "1. Enable this tool\n"
        "2. Reply to any video with /ss command\n"
        "3. Select how many screenshots you want (1-20)\n"
        "4. Bot will extract and send them\n\n"
        "**Note:** Screenshots are taken at random timestamps."
    )
    
    buttons = [
        InlineKeyboardButton(
            f"{'✅ Enabled' if active_tool == 'screenshot' else '❌ Disabled'}",
...
async def get_vt_audioremover_menu(user_id: int, menu_type: str = "main"):
    """Audio Track Remover Tool Menu"""
    settings = await db.get_user_settings(user_id)
    active_tool = settings.get("active_tool")
    
    caption = (
        "🎧 **Audio Track Remover**\n\n"
        "Remove specific audio tracks from videos!\n\n"
        "**How to use:**\n"
        "1. Enable this tool\n"
        "2. Reply to any video with /tracks command\n"
        "3. Select the audio track you want to remove\n"
        "4. Confirm and bot will process\n\n"
        "**Note:** All subtitles will also be removed during processing."
    )
    
    buttons = [
        InlineKeyboardButton(
            f"{'✅ Enabled' if active_tool == 'audioremover' else '❌ Disabled'}",
...
async def get_vt_hdcover_menu(user_id: int, menu_type: str = "main"):
    """HD Cover/Thumbnail Tool Menu"""
    settings = await db.get_user_settings(user_id)
    active_tool = settings.get("active_tool")
    
    caption = (
        "🖼️ **HD Cover/Thumbnail Tool**\n\n"
        "Add custom HD covers to your videos!\n\n"
        "**How to use:**\n"
        "1. Enable this tool\n"
        "2. Send a photo (this will be your cover)\n"
        "3. Send a video (cover will be automatically added)\n"
        "4. Use /removethumb to clear saved cover\n\n"
        "**Note:** Your last sent photo will be used as cover for all videos."
    )
    
    buttons = [
        InlineKeyboardButton(
            f"{'✅ Enabled' if active_tool == 'hdcover' else '❌ Disabled'}",
