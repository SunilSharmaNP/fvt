...
async def handle_screenshot_command(client: Client, message: Message):
    ...
    try:
        ...
        await message.reply_text(
            ...
            "Select the number of random screenshots to generate:",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("2 Screenshots", callback_data="ss_2"),
                    InlineKeyboardButton("5 Screenshots", callback_data="ss_5"),
                ],
                [
...
async def handle_screenshot_callback(client: Client, callback_query):
    ...
    try:
        ...
        # Create task log
        log_id = await log_simple_task(client, user, "Screenshot Generator", f"{ss_count} screenshots")
        ...
        if not info or not info.get('duration'):
            os.remove(file_path)
            await log_simple_complete(client, log_id, False, "Could not read video duration")
        ...
        if safe_end <= safe_start:
            os.remove(file_path)
            await log_simple_complete(client, log_id, False, "Video too short")
        ...
        # Get custom caption from video if exists
        original_caption = video_msg.caption or ""
        base_caption = f"📸 **Random Screenshot**\n\n"
        if original_caption:
            base_caption += f"{original_caption}\n\n"
        
        for i, (img, tstamp) in enumerate(output_files):
            caption = base_caption + f"🕒 **Timestamp:** `{tstamp}`\n📊 **Screenshot:** {i+1}/{len(output_files)}"
