# modules/processor.py (v6.12 — Final Sync)
# ✅ [CRITICAL FIX] डुप्लिकेट 'process_task' (लाइन 21) को हटा दिया गया।
# ✅ 'process_task' (लाइन 397) को 'bot.py' (v6.3) के साथ सिंक किया गया।
# ✅ [FIX] `_process_rename` को `shutil.move` और `asyncio.to_thread` का उपयोग करने के लिए ठीक किया गया।
# ✅ [FIX] `_process_mediainfo` में `shutil.which('mediainfo')` जाँच जोड़ी गई।
...
import json
import shlex
import shutil  # v6.12: `which` और `move` के लिए जोड़ा गया
import functools  # v6.12: `partial` के लिए जोड़ा गया
...
# ===================================================================
# 3. TOOL-SPECIFIC PROCESSORS (v6.7 से)
# ===================================================================
...
async def _process_mediainfo(status_message, task_id, downloaded_files):
    ...
    try:
        # ✅ [FIX v6.12] `shutil.which` जाँच जोड़ी गई
...
async def _process_rename(user_id, task_id, downloaded_files, settings,
                          progress_cb):
    # ✅ [FIX v6.12] `shutil.copy2` को `shutil.move` + `asyncio.to_thread` से बदल दिया गया
    input_file = downloaded_files[0]
...
async def _process_extract_thumb(user_id, task_id, downloaded_files, settings,
    ...
    # v6.11: `ffmpeg.extract_thumbnail` को कॉल करें
    success, msg, output_path = await ffmpeg.extract_thumbnail(
        input_file, output_dir, mode, task_id, user_id, progress_cb)
    return success, msg, output_path if success else None


# ---------------------- MAIN ROUTER (v6.5-style) ---------------------- #
async def process_task(client, user_id, task_id, downloaded_files,
    ...
    यह `settings` को आंतरिक (internally) रूप से फ़ेच करता है।
    """
    try:
        ...
        if tool == "merge":
            ...
            # ✅ [FIX v6.12] `settings` आर्ग्युमेंट यहाँ आवश्यक नहीं है
        ...
        if not success:
            ...
            raise Exception(
                f"Processing successful but no output file was returned ({msg})"
            )
        ...
        try:
            ...
            logger.error(f"Could not edit status message for {task_id}: {esm}")
